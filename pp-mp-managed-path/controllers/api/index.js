...
const ManagedPathMerchantModel = require('../../models/managed-path')
...
module.exports = (router) => {
	...
	router.get('/newMerchant', (req, res) => {
		const merchantObj = new ManagedPathMerchantModel()
	...
	router.post('/merchant-accounts', (req, res) => {
		getAccessToken(paypalConfig, (err, credentials) => {
			if(err) {
				console.log('ERROR')
				console.log(err)
				...
				request(options, (err, response, body) => {
					if(err) {
						console.log('ERROR in POST MERCHANT-ACCOUNTS')
						console.log(err)
						res.json(err)
					} else {
						res.json(body)
