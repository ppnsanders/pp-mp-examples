...
angular.module('ppMpBillingAgreements').service('orderServiceModel', function ($http, $cookies) {
	...
	function createOrder() {
		...
		return $http.post(reqUrl, model.order, config).then((response) => {
			...
			setTimeout(() => {
				...
				$('#billingAgreementButton').show()
				$('#orderResponseJson').show()
			}, 500)
		})
	}

	function setBillingAgreementTokenRequest() {
		model.billingAgreementTokenRequest.payer = {}
		model.billingAgreementTokenRequest.payer.payment_method = "paypal"
		model.billingAgreementTokenRequest.plan = {}
		model.billingAgreementTokenRequest.plan.type = "CHANNEL_INITIATED_BILLING"
		model.billingAgreementTokenRequest.plan.merchant_preferences = {}
		model.billingAgreementTokenRequest.plan.merchant_preferences.return_url = "https://localhost.com:8000/return"
		model.billingAgreementTokenRequest.plan.merchant_preferences.cancel_url = "https://localhost.com:8000/cancel"
		$('#orderRequestObject').hide()
		$('#billingAgreementFields').show()
		setTimeout(() => {
			$('#orderResponseObject').hide()
			$('#billingAgreementRequest').show()
			$('#createBillingAgreementButton').show()
		}, 100)
	}

	function createBillingAgreementToken() {
		$('#billingAgreementFields').hide('slide')
		$('#createBillingAgreementButton').hide('slide')
		$('#billingAgreementTokenResponse').show('slide')
		$('#billingAgreementTokenResponseJson').hide()
		$('#billingAgreementTokenResponseLoading').show()
		const reqUrl = '/api/billing-agreements/create'
		const config = {
            'xsrfHeaderName': 'X-CSRF-TOKEN',
            'xsrfCookieName': 'XSRF-TOKEN'
        }
		return $http.post(reqUrl, model.billingAgreementTokenRequest, config).then((response) => {
			model.billingAgreementTokenResponse = response.data
			setTimeout(() => {
				$('#showPayPalButton').show()
				$('#billingAgreementTokenResponseJson').show()
				$('#billingAgreementTokenResponseLoading').hide()
			}, 500)
		})
	}
	...
	function showPayPalButton() {
		...
		$('#billingAgreementRequest').hide('slide')
		setTimeout(() => {
	...
	let model = {
		...
		createBillingAgreementToken: (model) => {
			return createBillingAgreementToken(model)
		},
		setBillingAgreementTokenRequest: (model) => {
			return setBillingAgreementTokenRequest(model)
		}
	}
